...
| F-0019 | P1 | **FIXED** | All 5 alert rules deployed + evaluating: SSLCertExpiring14Days/7Days/Expired + SystemdUnitFailed/Flapping (all health=ok). 1530 node_systemd_unit_state samples flowing; aide.service correctly flagged failed on master. Rule count 47→53. | master+lady | 2026-04-24 | `09-EVIDENCE/15_f010_f019_deploy_*.md`, `09-EVIDENCE/16_f026_node_exporter_systemd_*.md` | — |
...
| F-0026 | P3 | **FIXED** | Patched node-exporter compose on both hosts: added `--collector.systemd` + `--collector.systemd.private` flags, mounted `/run/systemd/private:ro`, set `user: root`. 1530 systemd samples flowing fleet-wide; aide.service correctly detected failed. Backups preserved. | master+lady | 2026-04-24 | `09-EVIDENCE/16_f026_node_exporter_systemd_*.md` | — |
